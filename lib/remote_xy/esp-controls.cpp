...
void esp_print_states(void) {
    fprintf(stdout, "Buttons[Green:%d Orange:%d Blue:%d Red:%d] Axes[Joystick:%s(z%d)]\n",
            esp_button_green.state, esp_button_orange.state, esp_button_blue.state, esp_button_red.state,
            esp_get_direction_str(esp_joystick.direction).c_str(), esp_joystick.zone);
}
