...
// Calibrated motor tuning values
...
DCMotor leftMotor (MOTOR_A_IN1_PIN, MOTOR_A_IN2_PIN, MOTOR_A_EN_PIN);
DCMotor rightMotor(MOTOR_B_IN1_PIN, MOTOR_B_IN2_PIN, MOTOR_B_EN_PIN);
...
void setup() {
    ...
    // Apply tuning values; omitted args would fall back to MotorConfig defaults
...
void loop() {
    ...
    // Update motors based on esp_controls state
    robot.drive(static_cast<Direction>(esp_get_axis(ESP_JOYSTICK)->direction),
                esp_get_axis(ESP_JOYSTICK)->zone);
